...
st.set_page_config(page_title="Fractional Lévy Process with Macro Intensity", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Fractional Lévy Process with Macro Intensity</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Fractional jumps | Macro‑dependent jump intensity | Expected jump return</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 FLP")
...
st.sidebar.markdown(f"**Hurst (H):** {config.HURST} | **Jump threshold:** {config.JUMP_THRESHOLD}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'fractional_levy_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>FLP score: {etf['flp_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized FLP Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized FLP Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Fractional Lévy Expected Jump Return (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Fractional Lévy process** generalises fractional Brownian motion with jumps.
        - We detect jumps as returns exceeding a threshold (e.g., 2%).
        - Jump intensity (probability) is modelled as a logistic function of macro variables (VIX, DXY, yields).
        - The score = predicted jump probability × expected jump size (μⱼ).
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the FLP scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Fractional Lévy Process | Macro‑dependent jump intensity")
